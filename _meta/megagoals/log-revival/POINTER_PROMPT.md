...
Workflow (hard rules):
- One PR per sub-goal. A sub-goal that is only un-PR'd local diff is unstarted. Stacked `gh`: 01, 04, 06 base on `main`; 02 on 01; 03 on 02; 07 on 06; 05 fan-in on `main` after 02, 03, 07 merge (see each file's PR base).
...
- Merge mode auto-bottom-up + gated-final: EVERY sub-goal is `auto`. Walk the stack bottom-up and merge each PR yourself once all auto-merge gates hold (own Done verified + `gh pr checks` green + not CHANGES_REQUESTED + a committed proof-of-done carrying CAPTURED evidence, never a bare "passes"), doing the retarget-then-delete dance on merge. Merge everything EXCEPT 05, the held-final: open 05 and STOP for Han (merging 05 publishes the first post). NEVER merge 05, a red-CI, or a CHANGES_REQUESTED PR.
...
Stop conditions: all sub-goals merged + the held-final 05 opened-and-awaiting-Han (success); 05 ready and held; every remaining sub-goal blocked on an unchanged prerequisite; or budget exhausted. Anything else: keep moving. When 05 is ready, open it, append a final summary to NOTES `## Event log` on the FIRST pause, then emit a `🛑 NEEDS APPROVAL: sub-goal 05 (held-final), STOP /goal MANUALLY` banner. On a blocked fast-stop, after the first summary emit only a short `🛑 LOOP BLOCKED: STOP /goal MANUALLY` banner.
